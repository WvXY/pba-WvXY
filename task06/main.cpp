...
//  const Eigen::Matrix3d n = stiffness * u01 * u01.transpose();
  const Eigen::Matrix3d ddC = (Eigen::MatrixXd::Identity(3,3) - u01 * u01.transpose()) / length;
  ddw[0][0] = stiffness * (dC[0] * dC[0].transpose() + C * ddC);
  ddw[1][1] = stiffness * (dC[1] * dC[1].transpose() + C * ddC);
  ddw[0][1] = stiffness * (dC[0] * dC[1].transpose() - C * ddC);
  ddw[1][0] = stiffness * (dC[1] * dC[0].transpose() - C * ddC);
